...
epochs = 150 # number of epochs to train the SRCNN model for
...
file = h5py.File('/home/harshubh/Desktop/SRCNN/inputs/train_mscale.h5')
...
def validate(model, dataloader, epoch):
    ...
    with torch.no_grad():
        for bi, data in tqdm(enumerate(dataloader), total=int(len(train_data)/dataloader.batch_size)):
            ...
            running_psnr += batch_psnr
        outputs = outputs.cpu()
        save_image(outputs, f"/home/harshubh/Desktop/SRCNN/outputs/val_sr{epoch}.png")
    final_loss = running_loss/len(dataloader.dataset)
    final_psnr = running_psnr/int(len(train_data)/dataloader.batch_size)
    return final_loss, final_psnr
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('/home/harshubh/Desktop/SRCNN/outputs/loss.png')
plt.show()
# psnr plots
plt.figure(figsize=(10, 7))
...
plt.ylabel('PSNR (dB)')
plt.legend()
plt.savefig('/home/harshubh/Desktop/SRCNN/outputs/psnr.png')
plt.show()
# save the model to disk
print('Saving model...')
torch.save(model.state_dict(), '/home/harshubh/Desktop/SRCNN/outputs/model.pth')
